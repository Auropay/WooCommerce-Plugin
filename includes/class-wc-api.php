...
function updateOrder( $data ) {
	...
	if ( !$order_id || !$order ) {
		...
		if ( 'InProcess' == $status || 'SUCCESS' == $status ) {
			...
			if ( isset( $account_token ) && '' != $account_token && null != $account_token ) {
				global $wpdb;
...
function deletePaymentToken( $data ) {
	...
	if ( $row ) {
		$data = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}woocommerce_payment_tokens WHERE token_id = %s LIMIT 1",
