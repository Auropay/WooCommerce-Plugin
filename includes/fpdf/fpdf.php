...
class FPDF {
	...
	public function Image( $file, $x = null, $y = null, $w = 0, $h = 0, $typePdf = '', $link = '' ) {
		...
		if ( !isset( $this->images[$file] ) ) {
			...
			if ( '' == $typePdf ) {
				...
				$typePdf = substr( $file, $pos + 1 );
			}
			$typePdf = strtolower( $typePdf );
			if ( 'jpeg' == $typePdf ) {
				$typePdf = 'jpg';
			}
			$mtd = '_parse' . $typePdf;
			if ( !method_exists( $this, $mtd ) ) {
				$this->Error( 'Unsupported image type: ' . $typePdf );
			}
			$info = $this->$mtd( $file );
	...
	protected function _loadfont( $font ) {
		...
		include_once $this->fontpath . $font;
		if ( !isset( $name ) ) {
	...
	protected function _httpencode( $param, $value, $isUTF8 ) {
		...
		if ( !empty( $userAgent ) && strpos( $userAgent, 'MSIE' ) !== false ) {
	...
	protected function _parsegif( $file ) {
		...
		$data = ob_get_clean();
		imagedestroy( $im );
		$f = fopen( tempnam( sys_get_temp_dir(), 'transaction' ), 'rb+' );
		if ( !$f ) {
	...
	protected function _putfonts() {
		...
		foreach ( $this->fonts as $k => $font ) {
			...
			if ( 'Core' == $typePdf ) {
				...
				$this->_put( '>>' );
				$this->_put( 'endobj' );
			} elseif ( 'Type1' == $typePdf || 'TrueType' == $typePdf ) {
				// Additional Type1 or TrueType/OpenType font
				$this->_newobj();
				$this->_put( '<</Type /Font' );
				$this->_put( '/BaseFont /' . $name );
				$this->_put( '/Subtype /' . $typePdf );
				...
				if ( !empty( $font['file'] ) ) {
					$s .= ' /FontFile' . ( 'Type1' == $typePdf ? '' : '2' ) . ' ' . $this->FontFiles[$font['file']]['n'] . ' 0 R';
				}
				$this->_put( $s . '>>' );
				$this->_put( 'endobj' );
			} else {
				// Allow for additional types
				$mtd = '_put' . strtolower( $typePdf );
				if ( !method_exists( $this, $mtd ) ) {
					$this->Error( 'Unsupported font type: ' . $typePdf );
				}
				$this->$mtd( $font );
			}
		}
