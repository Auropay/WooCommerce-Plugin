...
function Message( $txt, $severity = '' ) {
	if ( PHP_SAPI == 'cli' ) {
		if ( $severity ) {
			echo esc_html( $severity ) . ': ';
		}
		echo esc_html( $txt ) . "\n";
	} else {
		if ( $severity ) {
			echo '<b>' . esc_html( $severity ) . '</b>: ';
		}
		echo esc_html( $txt ) . '<br />';
	}
}
...
function MakeDefinitionFile( $file, $typePdf, $enc, $embed, $subset, $map, $info ) {
	$s = "<?php\n";
	$s .= '$typePdf = \'' . $typePdf . "';\n";
...
// Generate a font definition file
	...
	MakeDefinitionFile( $basename . '.php', $typePdf, $enc, $embed, $subset, $map, $info );
