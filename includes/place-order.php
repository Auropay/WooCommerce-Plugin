...
function addStyle()
{
    wp_enqueue_style('hp_front_styles', WC_HP_PLUGIN_URL . '/assets/front-style.css');
}
...
function submitedOrderData()
{
    ...
    if (isset($_POST['order_action']) && $_POST['order_action'] == 'pay_for_order') {
...
function showCustomButton($button)
{
    ...
    if ($chosen_payment_method == WC_HP_PLUGIN_NAME) {
    ...
    <div class="timeline-event" id="c_step2" style="display:none;">      
      <div class="timeline-event-content ">
        <div class="timeline-event-title">
        <img src="' . WC_HP_PLUGIN_URL . '/assets/images/creating-payment-form.png" width="400" height="200" /> <br />
        </div>
        <div class="timeline-event-description">
        <p style="color:#0000ff;">One step more <br /> Creating your payment form</p>
        </div>
      </div>
    </div>    
 ...
 * @return string
 */
function showCustomPayButton($button)
{
    ...
    $js_event = "";
    $button = '<input type="button"
    onClick="pay_order(' . $order_pay_id . ')"
    class="button alt" name="woocommerce_checkout_place_order"
    id="place_order" value="' . esc_attr($order_button_text) . '" 
    data-value="' . esc_attr($order_button_text) . '" />
    <div id="loading" style="" > </div>';
...
function wpbHookJavascript()
{
    ...
    ?>
    <script>
        var expiry_min = <?php echo $expiry; ?>;
        ...
        function alertFunc() {
            //alert("alertFunc!");
            var delimg = "<img src='<?php echo WC_HP_PLUGIN_URL ?>/assets/images/close.png' width='20' height='20' onClick='hide_iframe()' style='cursor:pointer' >";
        ...
        function process_order() {
            ...
            jQuery('.woocommerce-form-coupon-toggle').append('<div class="woocommerce-NoticeGroup woocommerce-NoticeGroup-checkout"></div>');
                ...
                if (validate_required) {
                    if ('checkbox' === $this.attr('type') && !$this.is(':checked')) {
                        ...
                        jQuery.each($this, function(key, value) {
                            if (value.id == 'billing_first_name' || value.id == 'billing_last_name' || value.id == 'billing_postcode' || value.id == 'billing_phone' || value.id == 'billing_email' || value.id == 'billing_address_1' || value.id == 'billing_city') {
                                $div_data += '<li data-id="' + value.id + '"><strong>Billing ' + jQuery("label[for='" + value.id + "']").text() + '</strong> is a required field.</li>';
                                $parent.removeClass('woocommerce-validated').addClass('woocommerce-invalid woocommerce-invalid-required-field');
                                validated = false;
                                any_invalid = true;
                            }
                        ...
                        jQuery.each($this, function(key, value) {
                            if (value.id == 'billing_postcode') {
                                if (pincode_regex.test(value.value) === false) {
                                    $div_data += '<li data-id="' + value.id + '"><strong>Billing ' + jQuery("label[for='" + value.id + "']").text() + '</strong> is not valid.</li>';
                                    $parent.removeClass('woocommerce-validated').addClass('woocommerce-invalid woocommerce-invalid-required-field');
                                    validated = false;
                                    any_invalid = true;
                                }
            ...
            jQuery.ajax({
                ...
                success: function(result) {
                    jQuery('#c_step1').css('display', 'none');
                    jQuery('#c_step2').css('display', 'block');
                    ...
                    jQuery.ajax({
                        ...
                        success: function(result) {
                            ...
                            if (result != 'error') {
                                jQuery('#payment').append('<div class="del-img" id="close_iframe" style="display:none">' + delimg + '</div>');
                                if (jQuery('#hp_iframe').length == 0) {
                                    jQuery('#payment').append('<iframe src="' + result + '" name="hp_iframe" id="hp_iframe" scrolling="yes" frameborder=0 class="iframe-cs" style="display:none"></iframe>');
                                }
                                var myVar;
                                myVar = setTimeout(alertFunc, 5000);
                            } else {
                                jQuery('#payment').append('<div id="err-msg">Error when loading the payment form, please contact support team!</div>');
        ...
        function pay_order(order_id) {
            ...
            jQuery.ajax({
                ...
                success: function(result) {
                    ...
                    if (result != 'error') {
                        if (jQuery('#hp_iframe').length == 0) {
                            jQuery('#payment').append('<iframe src="' + result + '" name="hp_iframe" id="hp_iframe" scrolling="yes" frameborder=0 class="iframe-cs" ></iframe>');
                        } else {
                            jQuery('#hp_iframe').show();
                        }
                    } else {
